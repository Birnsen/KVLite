...
namespace KVL
{
    ...
    public partial class KVLite<T> : KJApi<T>
    {
        ...
        internal KVLite()
        {
            ...
            {
                _connections = createConnections(_ => (KVApi<T>) KVSqlite.CreateInMemory());
            }
            else if(typeof(T) == typeof(string) )
            {
                _connections = createConnections(_ => (KVApi<T>) KJSqlite.CreateInMemory());
            }
            else
            {
        ...
        internal KVLite(string directory)
        {
            ...
            if(typeof(T) == typeof(byte[]) )
            {
                _connections = createConnections(x => (KVApi<T>) KVSqlite.CreateWithFileInfo(new FileInfo(string.Format(filePath, x))));
            }
            else if(typeof(T) == typeof(string) )
            {
                _connections = createConnections(x => (KVApi<T>) KJSqlite.CreateWithFileInfo(new FileInfo(string.Format(filePath, x))));
            }
            else
            {
                throw new NotSupportedException($"Type {typeof(T).Name} is not supported!");
