...
namespace KVL
{
    internal class KVSqlite : KVApi<byte[]>
    {
        private readonly SQLiteConnection _connection;

        public static KVSqlite CreateWithFileInfo(FileInfo file)
        {
            return new KVSqlite(file.FullName);
        }

        public static KVSqlite CreateInMemory()
        {
            return new KVSqlite(":memory:");
        }

        private KVSqlite(string path)
        {
        ...
        public async Task Add(byte[] key, byte[] value)
        {
            using var cmd = _connection.CreateCommand();
            cmd.CommandText = $@"
        ...
        public async Task Add(IEnumerable<KeyValuePair<byte[], byte[]>> entries)
        {
        ...
        public async Task Update(byte[] key, byte[] value)
        {
            using var cmd = _connection.CreateCommand();
            cmd.CommandText = $@"
        ...
        public async Task Update(IEnumerable<KeyValuePair<byte[], byte[]>> entries)
        {
            using var trx = _connection.BeginTransaction();
            foreach(var e in entries)
            {
        ...
        public async Task<byte[]> Get(byte[] key)
        {
            ...
            return (byte[]) ret ?? throw new Exception("Key not found!");
        }

        public async IAsyncEnumerable<KeyValuePair<byte[], byte[] >> Get()
        {
            var pageCounter = 0;
            var entryCounter = 0;
            do
        ...
        private async IAsyncEnumerable<KeyValuePair<byte[], byte[] >> get(long page, int maxSize)
        {
